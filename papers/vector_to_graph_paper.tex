...
We evaluated retrieval quality using standard information retrieval metrics on 60 queries with annotated relevant documents. Table~\ref{tab:ir_metrics} shows the results.
...
\midrule
BaseParser & $N$ & 0 \\
EnhancedParser & $N + C$ & $C$ \\
Improvement & $+C$ & $+C$ \\
\bottomrule
\end{tabular}
\end{table}

The EnhancedTreeSitterParser adds $C$ call edges that the base parser does not detect, where $C$ depends on the number of function calls within the analyzed file. These call edges enable the system to understand which functions call other functions, enriching the knowledge graph with runtime behavioral relationships. The exact number of additional edges depends on the codebase size and call density of the indexed files.
